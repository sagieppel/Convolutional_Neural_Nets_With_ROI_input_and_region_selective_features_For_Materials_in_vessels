...
########################################Build Net#####################################################################################################################
    def build(self, rgb, Mask ,NUM_CLASSES,keep_prob):  #Build the fully convolutional neural network and load weight for decoder based on trained VGG16 network
        """
...
#-----------------------------Build network encoder based on VGG16 network and load the trained VGG16 weights-----------------------------------------
       #Layer 1
        ...
        self.MaskConv = tf.nn.bias_add(tf.nn.conv2d(tf.cast(Mask,tf.float32), W, [1, 1, 1, 1], padding="SAME"),B) # apply covolution add bias

        self.conv1_2 = self.conv_layer(tf.nn.relu(self.conv1_1*self.MaskConv), "conv1_2")#Build Convolution layer +Relu and load weights
